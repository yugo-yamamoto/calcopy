...
> 以下の画面イメージはすべてダミーデータです。図中の ①②③ は各手順の番号に対応しています。
...
Google カレンダーの DOM は難読化されていて変わりうるため、**実際の画面で確かめること**。
自作の DOM を模したテストは、通っているのに実物が壊れている状態を見逃すので置いていない。

```bash
# ブックマークを入れ替えたら、詳細ポップアップと検索結果一覧の両方で実行して
# トーストの内容を目で確認する
```

**ビルドしたら必ずブックマークを入れ替えること。** ブックマークバーのブックマークは
その時点のコードのスナップショットで、リビルドしても自動では更新されない。
